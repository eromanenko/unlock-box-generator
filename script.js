document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateLocks() {
        ...
        let htmlSpine = '';
        for(let i=0; i<3; i++) {
            const colorFront = i < d ? '#fff' : '#1b8a5d'; // active white, inactive green
            const colorSpine = i < d ? '#fff' : 'rgba(255,255,255,0.3)';
            htmlFront += `<span style="color:${colorFront}">${lockSVG}</span>`;
            htmlSpine += `<span style="color:${colorSpine}">${lockSVG}</span>`;
        }
